...
- **멀티 리포 지원**: vault root 및 하위 폴더의 git 저장소를 자동 탐지하고 개별 관리
...
프롬프트 폴더(기본: `IO_SecondBrain/프롬프트`)에 `.md` 파일을 추가하면 자동으로 액션 목록에 나타납니다. 파일 이름이 액션 이름이 됩니다.
...
| 프롬프트 폴더 | 프롬프트 템플릿 파일이 있는 폴더 경로 | `IO_SecondBrain/프롬프트` |
